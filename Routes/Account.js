                    ...
                    bcrypt.compare(req.body.Password, data.Password, function(err, resUser){
                        if(err){
                            ...
                            if(resUser===true){
                                ...
                                // Login thành công
                                // pr1 json lưu, pr2 privatekey, pr3 thời gian sống token, pr4 call back: 
                                jwt.sign({
                                    // thông tin lưu trữ trong token
                                    IdUser: data._id,
                                    ...
                                    Active: data.Active,
                                    RegisterDate: Date.now()
                                }, privatekey, {expiresIn:Math.floor(Date.now()/1000)+60*60*24*30}, function(err, token){
                                    if(err){
                                        res.json({kq:0, errMsg:err});
                                    }else{                                       
                                        // save tokens
                                        ...
                                        // lock hết các thiết bị khác đang đăng nhập -> Update token cua _id = false
                                    }
                                });
                            }else{
    ...
    app.post("/logout", function(req, res){
        Token.updateOne({Token:req.body.Token}, {State:false}, function(err){
            if(err){
                ...
                res.json({kq:1, errMsg:"Logout succsess"});
            }
        });
    });
}